...
WebRTC and WebSockets serve different real-time communication needs: WebRTC
enables peer-to-peer connections ideal for video, audio, and direct data
transfer, while WebSockets excel at client-server communication for general
real-time features. The two technologies are complementary, with WebSockets
often handling WebRTC signaling.
...
); ws.on('close', () => { if (currentRoom && rooms.has(currentRoom)) {
rooms.get(currentRoom).delete(ws); broadcast(currentRoom, { type: 'user-left',
userId: userId }); });

function broadcast(room, message, exclude) { if (rooms.has(room)) {
rooms.get(room).forEach(client => { if (client !== exclude && client.readyState
=== WebSocket.OPEN) { client.send(JSON.stringify(message)); (WebSocket +
WebRTC)// Client using both WebSocket and WebRTC class ConferenceClient {
...
async initialize() { await this.setupSignaling(); await this.setupLocalMedia();
this.setupUI(); }
...
async initialize() { // Get local media this.localStream = await
navigator.mediaDevices.getUserMedia({ video: true, audio: true // Set up
WebSocket handlers this.ws.onopen = () => { this.ws.send(JSON.stringify({ type:
'join-room', room: this.roomId, userId: this.userId })); };
...
async createPeerConnection(peerId, createOffer) { const pc = new
...
sendSignalpeerId, ice-candidatepeerssetpeerId, pc if (createOffer) { const offer
= await pc.createOffer(); await pc.setLocalDescription(offer);
this.sendSignal(peerId, { }

sendSignal(targetUserId, signal) { this.ws.send(JSON.stringify({ type: 'signal',
targetUserId: targetUserId, signal: signal })); }

sendChatMessage(message) { // Chat goes through WebSocket, not WebRTC
this.ws.send(JSON.stringify({ type: 'chat', message: message })); } }

// Export for testing module.exports = VideoChatServer;
...
connection. WebRTC enables peer-to-peer connections for video, audio,
and data directly between browsers. WebSockets need a server relay;
WebRTC can bypass the server after connection setup.

### Can I use WebSocket and WebRTC together?

Yes, this is common. WebSockets are typically used as the signaling
channel to exchange WebRTC session descriptions and ICE candidates.
Once the peer-to-peer connection is established, data flows directly
via WebRTC.

### Should I use WebSocket or WebRTC for chat?

Use WebSockets for text chat — they are simpler, more reliable, and
work through all firewalls. Use WebRTC only if you need video or audio
chat, or if peer-to-peer data transfer is specifically required.
...
While raw WebSocket or WebRTC implementation is complex, production
applications often benefit from using established libraries like
Socket.IO or commercial services that handle protocol complexities,
