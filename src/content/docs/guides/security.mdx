---
title: 'WebSocket Security: Auth, TLS, CSWSH & Rate Limiting'
description:
  'Secure your WebSocket connections. Covers authentication (JWT,
  token), TLS/WSS setup, CSWSH prevention, rate limiting, input
  validation, and OWASP best practices.'
keywords:
  - websocket security
...
faq:
  - q: 'What is Cross-Site WebSocket Hijacking (CSWSH)?'
    a: 'CSWSH is an attack where a malicious website opens a WebSocket
      connection to a target server using the victim browser cookies.
      Prevent it by validating the Origin header during the handshake
      and using CSRF tokens or custom headers that cross-origin
      requests cannot set.'
  - q: 'Should I use WSS (WebSocket Secure) in production?'
    a: 'Yes, always. WSS encrypts the connection with TLS, preventing
      eavesdropping and man-in-the-middle attacks. Most browsers and
      proxies handle WSS more reliably than plain WS. Configure TLS
      1.2+ with strong cipher suites.'
  - q: 'How do I authenticate WebSocket connections?'
    a: 'Pass a JWT or session token during the initial HTTP upgrade
      handshake, either as a query parameter or in cookies. Validate
      the token server-side before accepting the connection. For
      long-lived connections, implement token refresh over the
      WebSocket itself.'
  - q: 'How do I rate limit WebSocket connections?'
    a: 'Limit both new connections and messages per connection. Track
      connection counts per IP and per authenticated user. Implement
      a token bucket or sliding window algorithm for message rate
      limiting. Disconnect clients that exceed limits.'
---
...
:::note[Quick Answer]
Always use **WSS** (TLS) in production. Validate the **Origin header**
during the handshake to prevent CSWSH. Authenticate with **JWT tokens**
passed as a query parameter or cookie during the upgrade request.
Rate limit both connections and messages per client.
:::
...
For a comprehensive overview of WebSocket authentication methods and
best practices, see
[this essential guide to WebSocket authentication][ably-ws-auth].
...
- [WebSocket Protocol: RFC 6455](/guides/websocket-protocol/) -
  Protocol details including the security-relevant handshake
- [Nginx WebSocket Proxy](/guides/infrastructure/nginx/) -
  TLS termination and proxy security configuration
- [Kubernetes WebSocket Ingress](/guides/infrastructure/kubernetes/) -
  Network policies and TLS for WebSocket workloads
- [WebSocket Close Codes](/reference/close-codes/) -
  Understanding close codes for security event handling
- [WebSockets at Scale](/guides/websockets-at-scale/) -
  Rate limiting and connection management at scale

[ably-ws-auth]: https://ably.com/blog/websocket-authentication?utm_source=websocket-org&utm_medium=security
